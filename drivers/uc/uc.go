...
func (driver uc) Request(pathname string, method int, headers, query, form map[string]string, data interface{}, resp interface{}, account *model.Account) ([]byte, error) {
	u := "https://pc-api.yun.cn/1/clouddrive" + pathname
	req := base.RestyClient.R()
	req.SetHeaders(map[string]string{
		"Cookie":  account.AccessToken,
		"Accept":  "application/json, text/plain, */*",
		"Referer": "https://www.yun.cn/",
	})
...
func (driver uc) Get(pathname string, query map[string]string, resp interface{}, account *model.Account) ([]byte, error) {
	return driver.Request(pathname, base.Get, nil, query, nil, nil, resp, account)
}

func (driver uc) Post(pathname string, data interface{}, resp interface{}, account *model.Account) ([]byte, error) {
	return driver.Request(pathname, base.Post, nil, nil, nil, data, resp, account)
}

func (driver uc) GetFiles(parent string, account *model.Account) ([]model.File, error) {
...
func (driver uc) UpPre(file *model.FileStream, parentId string, account *model.Account) (UpPreResp, error) {
	now := time.Now()
	data := base.Json{
...
func (driver uc) UpHash(md5, sha1, taskId string, account *model.Account) (bool, error) {
	data := base.Json{
		"md5":     md5,
		"sha1":    sha1,
		"task_id": taskId,
...
func (driver uc) UpPart(pre UpPreResp, mineType string, partNumber int, bytes []byte, account *model.Account) (string, error) {
	//func (driver uc) UpPart(pre UpPreResp, mineType string, partNumber int, bytes []byte, account *model.Account, md5Str, sha1Str string) (string, error) {
...
func (driver uc) UpCommit(pre UpPreResp, md5s []string, account *model.Account) error {
	timeStr := time.Now().UTC().Format(http.TimeFormat)
...
func (driver uc) UpFinish(pre UpPreResp, account *model.Account) error {
	data := base.Json{
